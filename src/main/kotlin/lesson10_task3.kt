...
fun generatePassword(passwordSize: Int?): String { // Явно указали, что функция возвращает String
    ...
    val listOfSpecialCymbol = listOf("!", "\"", "#", "\\", "\$", "%", "&", "'", "(", ")", "*", "+", ",", "-", ".", "/")
    val rangeListOfSpecialCymbol = COUNT_NULL..(listOfSpecialCymbol.size - 1)
    ...
    for (i in COUNT_ONE..passwordSize!!) {
        if (i % 2 == 0) {
            ...
            password += listOfSpecialCymbol.get(rangeListOfSpecialCymbol.random())
        }
    }
